...
import { gcd } from '../../framework/math.js';
import { Face } from './math3d.js';
...
export const challenge = {
  ...
  solvePart1() {
    ...
    for (const step of this.steps) {
      ...
      for (let i = 0; i < step; i++) {
        ...
        do {
          if (dir.x !== 0) {
            next.x = mod(next.x + dir.x, this.map[next.y].length);
          } else {
            next.y = mod(next.y + dir.y, this.map.length);
          }
  ...
  detectCubeFaces() {
    ...
    this.faceSize = gcd(mapWidth, mapHeight);
  ...
  cubeWrap(pos, dir) {
    ...
    let nextDir = dir;
    const rotCount = mod((edgeB - edgeA) + 2, 4);
