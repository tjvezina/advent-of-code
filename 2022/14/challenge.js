...
export const challenge = {
  ...
  init() {
    ...
    this.bounds = structures.flatMap(x => x).reduce((bounds, pos) => ({
    ...
    this.map = new Array(this.bounds.yMax + 1).fill().map(_ => []);
  ...
  solvePart2() {
    ...
    this.map.push([]);
    this.map.push(new Array(SAND_START_X + floor + 1).fill().map(_ => ROCK));
